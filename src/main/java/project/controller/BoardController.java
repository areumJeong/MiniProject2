...
public class BoardController extends HttpServlet {
	...
	private BoardAdviceService bSvc = new BoardAdviceServiceImpl();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "list":			// /jw/bbs/board/list?p=1&f=title&q=검색
			...
			List<BoardAdvice> boardList = bSvc.getBoardAdviceList(page, field, query);
			request.setAttribute("boardList", boardList);
			
			// for pagination
			int totalItems = bSvc.getBoardAdviceCount(field, query);
		...
		case "insert":
			...
			if (method.equals("GET")) {
				...
				board = new BoardAdvice(title, content, sessUid);
				bSvc.insertBoardAdvice(board);
		...
		case "update":
			if (method.equals("GET")) {
				...
				title = request.getParameter("title");
				content = request.getParameter("content");
				board = new BoardAdvice(bid, title, content);
				
				bSvc.updateBoardAdvice(board);
